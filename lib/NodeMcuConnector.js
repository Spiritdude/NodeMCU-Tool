...
NodeMcuConnector.prototype.fetchDeviceInfo = function(cb){
    ...
    this.device.executeCommand(_luaCommandBuilder.command.nodeInfo, function(err, echo, data){
        if (err){
            ...
            // console.log(p);
            if (p.length < 8 || data.match(/attempt to call field/)) { 
                // let's try to probe the chipid, as older dev-esp32 nodemcu lacks node.info() as of 2018/01 (might change later)
                me.device.executeCommand(_luaCommandBuilder.command.chipid, function(err, echo, data){
                   if (err) {
                       ...
                       var m = data.match(/^0x(\w+)/);     // older esp32-firmware chipid (hex with '0x' prefix)?
                       if(m) {
